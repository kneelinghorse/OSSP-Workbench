...
class MultiAgentE2EDemo {
  ...
  /**
   * Load discovered agents from registry (best effort).
   * @private
   * @returns {Promise<Array>}
   */
  async _loadAgentsFromRegistry() {
    if (
      !this.registry ||
      typeof this.registry.getStats !== 'function' ||
      typeof this.registry.listAgentsByDomain !== 'function'
    ) {
      return [];
    }
    ...
    try {
      ...
      for (const domain of domains) {
        ...
        for (const agent of domainAgents || []) {
          if (!agent?.urn || seenUrns.has(agent.urn)) {
            continue;
          }
          seenUrns.add(agent.urn);
          agents.push(agent);
        }
      }
  ...
  /**
   * Ensure we have a non-empty discovered agent list for downstream steps.
   * @private
   * @returns {Promise<Array>}
   */
  async _ensureDiscoveredAgents() {
    if (this.discoveredAgents.length > 0) {
      return this.discoveredAgents;
    }

    const registryAgents = await this._loadAgentsFromRegistry();
    if (registryAgents.length > 0) {
      this.discoveredAgents = registryAgents;
      return this.discoveredAgents;
    }

    // Fall back to demo fixtures so isolated tests can still exercise the loop.
    this.discoveredAgents = [...DEMO_AGENTS];
    return this.discoveredAgents;
  }

  /**
   * Extract agent array from discovery results.
   * @private
   * @param {any} result
   * @returns {Array}
   */
  _discoveryAgents(result) {
  ...
  async step1AgentDiscovery() {
    ...
    try {
      ...
      // Register demo agents (ignore duplicate registrations).
      if (this.registry && typeof this.registry.registerAgent === 'function') {
      ...
      // Discover agents by domain when the discovery service supports it.
      const discoveredByDomain = [];
      if (this.discovery && typeof this.discovery.discoverByDomain === 'function') {
        ...
        discoveredByDomain.push(
          ...this._discoveryAgents(aiAgents),
          ...this._discoveryAgents(dataAgents),
          ...this._discoveryAgents(apiAgents)
        );
      }

      const registryAgents = await this._loadAgentsFromRegistry();
      const fallbackAgents = discoveredByDomain.length === 0 && registryAgents.length === 0
        ? DEMO_AGENTS
        : [];

      const byUrn = new Map();
      for (const agent of [...discoveredByDomain, ...registryAgents, ...fallbackAgents]) {
        if (agent?.urn) {
          byUrn.set(agent.urn, agent);
        }
      }
      this.discoveredAgents = Array.from(byUrn.values());
  ...
  async step2A2ACommunication() {
    ...
    try {
      ...
      const startTime = performance.now();
      const a2aResults = [];
      const agents = await this._ensureDiscoveredAgents();
    ...
    try {
      ...
      for (const agent of agents) {
        try {
          ...
          const mcpEndpoint = agent?.endpoints?.mcp || agent?.endpoints?.api || 'node';
          await this.mcpClient.connect(mcpEndpoint);
    ...
    try {
      ...
      for (const agent of agents) {
        try {
          ...
          const discoveredAgent = typeof this.discovery?.getAgent === 'function'
            ? await this.discovery.getAgent(agent.urn)
            : agent;
          if (!discoveredAgent) {
    ...
    try {
      ...
      try {
        ...
        errorTests.push({
          ...
          success: String(circuitStatus.state || '').toUpperCase() === 'OPEN',
