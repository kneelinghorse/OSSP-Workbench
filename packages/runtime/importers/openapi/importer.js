...
import { detectPagination, detectLongRunning } from './patterns.js';
import { extractExtensions, preserveValuedExtensions } from './extensions.js';
  ...
  async _parseSpec(source) {
    try {
      ...
      if (error.message.includes('fetch') || error.message.includes('network') || 
          ...
          error.message.includes('timeout') || error.message.includes('CORS')) {
        throw new Error(`Network error fetching OpenAPI spec from ${source}: ${error.message}. Try using a local file instead.`);
      }
      ...
      try {
        ...
        if (fallbackError.message.includes('fetch') || fallbackError.message.includes('network')) {
          throw new Error(`Network error accessing ${source}: ${fallbackError.message}. Please check the URL or use a local file.`);
        }
        throw new Error(`Failed to parse OpenAPI spec from ${source}: ${error.message}. Fallback also failed: ${fallbackError.message}`);
      }
    }
  }
