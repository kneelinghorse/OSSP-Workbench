...
import { generateEventURN } from './urn-generator.js';
import { detectProtocolBindings } from './binding-detector.js';
import { detectEventPII } from './pii-detector.js';
import { extractPayloadSchema } from './schema-utils.js';
import { detectEventPatterns } from './patterns.js';

// IMPORTANT: Lazy load parser to avoid 2.85MB CLI startup penalty
let Parser;
async function getParser() {
  if (!Parser) {
    const { Parser: P } = await import('@asyncapi/parser');
    Parser = P;
  }
  return Parser;
}
