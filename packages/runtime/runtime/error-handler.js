...
import {
  A2AError as A2AClientError,
  AuthError as A2AClientAuthError,
  TimeoutError as A2AClientTimeoutError,
  NetworkError as A2AClientNetworkError,
  RetryError as A2AClientRetryError
} from './a2a-types.js';
...
export class ErrorHandler {
  ...
  handleError(error, context = {}) {
    ...
    if (error instanceof RuntimeError) {
      ...
      error instanceof A2AClientAuthError ||
      error instanceof A2AClientTimeoutError ||
      error instanceof A2AClientNetworkError ||
      error instanceof A2AClientRetryError
    ) {
      // Preserve A2A error identity so instanceof checks remain stable.
      typedError = error;
  ...
  _mapError(error, context) {
    ...
    if (lowerMessage.includes('circuit') || lowerMessage.includes('breaker')) {
      return new CircuitBreakerError(message, error, null, context);
    }

    if (lowerMessage.includes('retry') || lowerMessage.includes('attempts')) {
    ...
    if (lowerMessage.includes('a2a') || lowerMessage.includes('agent')) {
      return new A2AError(message, error, context);
    }

    if (lowerMessage.includes('registry') || lowerMessage.includes('urn')) {
      return new RegistryError(message, error, context);
    }

    if (lowerMessage.includes('discovery') || lowerMessage.includes('discover')) {
