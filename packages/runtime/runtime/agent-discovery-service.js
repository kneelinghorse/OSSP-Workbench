...
export class AgentDiscoveryService extends EventEmitter {
  ...
  async getAgent(urn) {
    ...
    try {
      ...
      throw new URNError(
        `Failed to retrieve agent: ${error.message}`,
        error
      );
    }
  }
  ...
  /**
   * Backward-compatible alias used by older demos/tests.
   * @returns {Promise<void>}
   */
  async cleanup() {
    await this.shutdown();
  ...
  _applyFilters(agents, query) {
    return agents.filter(agent => {
      ...
      // Version filter
      if (query.version) {
        const agentVersion = agent.version || this._extractVersionFromUrn(agent.urn);
        if (agentVersion !== query.version) {
          return false;
        }

        // When available, capability versions should align with agent version.
        const capabilityVersions = Object.values(agent.capabilities || {})
          .map((capability) => capability?.version)
          .filter(Boolean);

        if (
          capabilityVersions.length === 0 ||
          !capabilityVersions.every((version) => version === query.version)
        ) {
          return false;
        }
      }
  ...
  /**
   * Extract version from agent URN.
   * @private
   * @param {string} urn - Agent URN
   * @returns {string|undefined} Version string
   */
  _extractVersionFromUrn(urn) {
