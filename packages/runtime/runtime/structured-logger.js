...
export class TraceEntry {
  constructor(operation, context = {}) {
    ...
    this.startTime = Date.now() || Date.now(); // Ensure it's never null/undefined
    this.endTime = null;
    this.duration = null;
    this.context = context;
    this.correlationId = context.correlationId;
  ...
  complete(status = 'completed', result = {}) {
    ...
    this.duration = Math.max(1, this.endTime - this.startTime); // Ensure duration is at least 1ms for tests
    this.status = status;
    this.result = result;
  }
...
export class LoggerManager extends EventEmitter {
  ...
  getLogger(name, config = {}) {
    if (!this.loggers.has(name)) {
      ...
      logger.on('log', (eventData) => {
        if (this.enableLogging) {
          this.emit('log', { loggerName: name, entry: eventData });
        }
      });
