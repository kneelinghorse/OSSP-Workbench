...
export class MCPClient extends EventEmitter {
  constructor(options = {}) {
    ...
    this.retryPolicy = createRetryPolicy({
      maxRetries: options.maxRetries ?? options.retries ?? 3,
  ...
  async connect(endpointOrOptions = {}) {
    ...
    if (typeof endpointOrOptions === 'string') {
      this.endpoint = parseEndpoint(endpointOrOptions);
    } else if (endpointOrOptions && typeof endpointOrOptions === 'object') {
      if (endpointOrOptions.endpoint !== undefined) {
        this.endpoint = parseEndpoint(endpointOrOptions.endpoint);
        const { endpoint, ...rest } = endpointOrOptions;
        openOptions = rest;
      } else if (
        endpointOrOptions.command !== undefined ||
        endpointOrOptions.args !== undefined ||
        endpointOrOptions.env !== undefined ||
        endpointOrOptions.type !== undefined
      ) {
        this.endpoint = parseEndpoint(endpointOrOptions);
      } else {
        openOptions = endpointOrOptions;
      }
    }
