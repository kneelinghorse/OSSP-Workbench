...
class OpenAPIParser {
  ...
  async _parseRawSpec(source) {
    ...
    try {
      // Fast path: local JSON file parsing avoids stream parser startup overhead.
      if (
  ...
  async _parseWithSwaggerParser(rawSpec) {
    try {
      ...
      // Fast path for small/local specs with no refs to avoid SwaggerParser cold-start overhead.
      if (this._canUseFastPath(rawSpec)) {
        this.progressTracker.completeStage('resolving_local_refs');
        return rawSpec;
      }
  ...
  /**
   * Determine whether SwaggerParser validation/dereference can be safely skipped.
   * @private
   */
  _canUseFastPath(rawSpec) {
    if (!rawSpec || typeof rawSpec !== 'object') {
      return false;
    }

    const hasRequiredShape =
      typeof rawSpec.openapi === 'string' &&
      rawSpec.info &&
      typeof rawSpec.info === 'object' &&
      rawSpec.paths &&
      typeof rawSpec.paths === 'object';

    if (!hasRequiredShape) {
      return false;
    }

    // Keep parser validation for specs with external refs. For local refs,
    // fast-path parsing is sufficient for endpoint/schema extraction.
    return !this._containsExternalRef(rawSpec);
  }

  /**
   * Check recursively whether an object contains external $ref fields.
   * @private
   */
  _containsExternalRef(value, seen = new Set()) {
    ...
    for (const entry of Object.values(value)) {
      if (this._containsExternalRef(entry, seen)) {
        return true;
      }
    }
