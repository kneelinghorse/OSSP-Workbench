...
import { findPIIExposingEndpoints } from '../core/graph/pii-tracer.js';
  ...
  validateURNReferences(manifest) {
    ...
    for (const { urn, field } of urns) {
      ...
      if (!formatValidation.valid) {
        const isVersionOptionalField =
          field === 'catalog.urn' || field === 'service.urn' || field === 'urn';
        const severity = formatValidation.reason === 'URN missing version' && isVersionOptionalField
          ? Severity.WARNING
          : Severity.ERROR;
  ...
  _validateURNFormat(urn) {
    ...
    if (!parsed) {
      return {
        ...
        suggestion: 'Use format: urn:proto:<kind>:<authority>/<id>@<version> or urn:ossp:<kind>:<authority>:<id>:v<version>'
      };
    }
    if (!parsed.version) {
