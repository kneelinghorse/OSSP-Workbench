 ...
 * - urn:proto:<kind>:<id>[@<version>]
 * - urn:ossp:<kind>:<authority>:<id>[:v<version>]
 *
 * Examples:
...
function parseURN(urn) {
  ...
  // Format: urn:proto:<kind>:<authority>/<id>[@<version>] or urn:proto:<kind>:<id>[@<version>]
  const protoPattern = /^urn:proto:([^:]+):(?:([a-zA-Z0-9._-]+)\/)?([a-zA-Z0-9._-]+(?:\/[a-zA-Z0-9._-]+)*)(?:@(.+))?$/;
  const protoMatch = urn.match(protoPattern);

  if (protoMatch) {
    const [, kind, authority, id, version] = protoMatch;
  ...
  // Legacy format: urn:ossp:<kind>:<authority>:<id>[:v<version>]
  const osspPattern = /^urn:ossp:([^:]+):([a-zA-Z0-9._-]+):([a-zA-Z0-9._-]+)(?::v?([a-zA-Z0-9._-]+))?$/;
  const osspMatch = urn.match(osspPattern);

  if (osspMatch) {
    const [, kind, authority, id, version] = osspMatch;
    if (!VALID_KINDS.has(kind)) {
      return null;
    }
...
function normalizeURN(urn) {
  ...
  if (parsed.namespace === 'ossp') {
    return `urn:ossp:${parsed.kind}:${parsed.authority}:${parsed.id}`;
  }
  ...
  return `urn:proto:${parsed.kind}:${parsed.id}`;
}
 ...
 */
function buildURN({ kind, authority, id, version = null, namespace = 'proto' }) {
  if (!VALID_KINDS.has(kind)) {
    throw new Error(`Invalid URN kind: ${kind}`);
  }

  if (namespace === 'ossp') {
    let urn = `urn:ossp:${kind}:${authority}:${id}`;
    if (version) {
      urn += `:v${String(version).replace(/^v/, '')}`;
    }
    return urn;
  }
