...
import { ProtocolGraph } from '../graph/index.js';
import { OverrideEngine } from '../overrides/index.js';
import { EventSectionGenerators } from './event-section-generator.js';
...
class GovernanceGenerator {
  ...
  /**
   * Evaluate a breaking change against governance policy.
   * Returns a deterministic policy decision object used by governance checks.
   */
  enforceBreakingChangePolicy(change = {}) {
    ...
    const isBreaking =
      ...
      impact === 'high' ||
      severity === 'high' ||
      change.migrationRequired === true;

    if (!isBreaking) {
      return {
        approved: true,
        reason: 'no breaking change detected',
    ...
    return {
      approved: false,
      reason: 'breaking change policy requires explicit approval before merge',
  ...
  /**
   * Determine whether a change requires approval and identify approvers.
   */
  requireApproval(change = {}) {
    const severity = String(change.severity || '').toLowerCase();
    const impact = String(change.impact || '').toLowerCase();
    const affectedConsumers = Number(change.affectedConsumers || 0);
    const required = severity === 'high' || impact === 'high' || affectedConsumers > 0;

    return {
      required,
      approvers: required ? ['protocol-owner', 'platform-governance'] : [],
      deadline: required
  ...
  /**
   * Track policy violations for audit and escalation.
   */
  trackViolation(violation = {}) {
    this._violationCounter += 1;
    const violationId = `violation-${String(this._violationCounter).padStart(4, '0')}`;
