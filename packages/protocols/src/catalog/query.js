...
/**
 * CatalogQuery - lightweight semantic query adapter for URNCatalogIndex
 * - Parses simple natural language prompts and routes them to
 *   the corresponding capability-based agent discovery queries.
 *
 * Supported patterns (case-insensitive):
 * - "agents with tool <name>"
 * - "agents by resource <uri>" or "resource <uri>"
 * - "agents for workflow <urn>" or workflow URN in text
 * - "agents for api <urn>" or API URN in text
 * - If API URN detected without direct matches, tries via workflow traversal
 */
export class CatalogQuery {
  /**
   * @param {import('./index').URNCatalogIndex} catalog
   */
  constructor(catalog) {
   ...
   * Routes a free-form string to specific discovery queries.
   * Returns unique agent URNs and timing metadata.
   *
   * @param {string} query
  ...
  findAgentsCapableOf(query) {
    ...
    // 1) Extract explicit URNs in the query (workflow/api)
    const urnRegex = /(urn:(?:proto|protocol):(?:workflow|api):[^\s'"\\]+)/g;
    const urns = Array.from(text.matchAll(urnRegex)).map(m => m[1]);

    for (const urn of urns) {
      if (urn.includes(':workflow:')) {
        const r = this.catalog.findAgentsByWorkflow(urn);
        for (const u of r.results) results.add(u);
      } else if (urn.includes(':api:')) {
        // Try direct API relationship first
        const direct = this.catalog.findAgentsByAPI(urn);
        for (const u of direct.results) results.add(u);
        // Also include via workflow traversal
        const via = this.catalog.findAgentsByAPIViaWorkflow(urn);
        for (const u of via.results) results.add(u);
      }
    }

    // 2) Tool pattern: "with tool <name>" or "tool <name>"
    //    Prefer quoted capture, fallback to unquoted token
    let match = text.match(/tool\s+["']([^"']+)["']/);
    if (!match) match = text.match(/(?:with\s+)?tool\s+([a-z0-9_.\-]+)/);
    if (match && match[1]) {
      const r = this.catalog.findAgentsByTool(match[1]);
      for (const u of r.results) results.add(u);
    }

    // 3) Resource pattern: "resource <uri>" with common URI schemes
    const res = text.match(/resource\s+((?:https?:\/\/|file:\/\/)[^\s'"\\]+)/);
    if (res && res[1]) {
      const r = this.catalog.findAgentsByResource(res[1]);
      for (const u of r.results) results.add(u);
    }
    ...
    return { results: list, count: list.length, took };
  }
}
...
export function queryByTag(tag, indexes, artifacts) {
  ...
  const urns = indexes.byTag.get(tag);
  if (!urns || urns.size === 0) {
    return {
      results: [],
      count: 0,
  ...
  const results = new Array(urns.size);
  let count = 0;
  for (const urn of urns) {
    const artifact = artifacts.get(urn);
    if (artifact !== undefined) {
      results[count++] = artifact;
    }
  }
