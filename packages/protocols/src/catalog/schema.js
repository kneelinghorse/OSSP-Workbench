 ...
 * @typedef {'event-protocol'|'data-protocol'|'api-protocol'|'ui-protocol'|'workflow-protocol'|'agent-protocol'} ProtocolType
 */
...
export const catalogIndexSchema = {
  ...
  properties: {
    ...
    artifacts: {
      type: 'object',
      patternProperties: {
        '^urn:protocol:(event|data|api|ui|workflow|agent):[a-z][a-z0-9]*\\.[a-z][a-z0-9]*:\\d+\\.\\d+\\.\\d+$': {
          type: 'object',
          required: ['urn', 'name', 'version', 'namespace', 'type', 'manifest', 'dependencies', 'metadata'],
          properties: {
            urn: {
              type: 'string',
              pattern: '^urn:protocol:(event|data|api|ui|workflow|agent):[a-z][a-z0-9]*\\.[a-z][a-z0-9]*:\\d+\\.\\d+\\.\\d+$'
            },
            name: {
              type: 'string',
              minLength: 1
            ...
            namespace: {
              type: 'string',
              pattern: '^urn:protocol:(event|data|api|ui|workflow|agent)$'
            },
            type: {
              type: 'string',
              enum: ['event-protocol', 'data-protocol', 'api-protocol', 'ui-protocol', 'workflow-protocol', 'agent-protocol']
            },
            manifest: {
              type: 'string',
              format: 'uri'
