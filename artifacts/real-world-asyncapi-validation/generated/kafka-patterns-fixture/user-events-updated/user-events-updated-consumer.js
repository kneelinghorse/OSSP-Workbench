...
/**
 * Consumer for user.events.updated
 * Purpose: Event channel: user.events.updated
 *
 * Governance:
 ...
 * - Error resilience: retry policy exponential (maxRetries=5, backoffMs=5000, multiplier=2)
 * - Poison pill handling: skip undecodable payloads after 5 retries
 * - Circuit breaker: threshold=6, resetTimeoutMs=10000
 ...
 */
export class UserEventsUpdatedConsumer {
  kafka;
  consumer;
  monitoring;
  auth = {};
  ...
  constructor(config) {
    ...
    this.resilienceConfig = {
      ...
      retry: {
        ...
        backoffMs: this.toNumber(config.retry?.backoffMs, process.env.KAFKA_RETRY_BACKOFF_MS, 5000),
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'user.events.updated';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'user.events.updated-consumer',
      brokers
    };
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'kafka',
      consumer: 'UserEventsUpdatedConsumer',
      ...data
    };
  ...
  async handleError(error, payload) {
    ...
    // Route to DLQ: user.events.updated.dlq
    await this.sendToDLQ(payload, error);
  }
    ...
    await this.dlqProducer.send({
      ...
      messages: [{
        ...
        headers: {
          ...
          'x-original-topic': 'user.events.updated'
        }
      }]
    });
  }
