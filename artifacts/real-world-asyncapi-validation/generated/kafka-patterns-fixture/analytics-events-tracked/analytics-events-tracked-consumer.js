...
/**
 * Consumer for analytics.events.tracked
 * Purpose: Event channel: analytics.events.tracked
 *
 * Governance:
 * - PII fields: None
 * - DLQ configured: ⚠️ No
 ...
 * - ℹ️ Retry: Retry policy configured but no backoff specified
 * - Error resilience: retry policy fixed (maxRetries=3, backoffMs=1000, multiplier=1)
 ...
 */
export class AnalyticsEventsTrackedConsumer {
  kafka;
  consumer;
  monitoring;
  auth = {};
  ...
  constructor(config) {
    ...
    this.resilienceConfig = {
      ...
      retry: {
        ...
        backoffMultiplier: this.toNumber(config.retry?.backoffMultiplier, process.env.KAFKA_RETRY_BACKOFF_MULTIPLIER, 1),
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'analytics.events.tracked';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'analytics.events.tracked-consumer',
      brokers
    };
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'kafka',
      consumer: 'AnalyticsEventsTrackedConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs, payload) {
    ...
    metrics?.incrementMessagesProcessed?.({
      ...
      event: 'analytics.events.tracked'
    });
