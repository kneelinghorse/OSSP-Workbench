...
/**
 * Consumer for user.events.deleted
 * Purpose: Event channel: user.events.deleted
 *
 * Governance:
 * - PII fields: [userId]
 * - DLQ configured: ⚠️ No
 ...
 * - ⚠️ WARNING: PII events with retries must declare a DLQ
 * - Error resilience: retry policy exponential (maxRetries=5, backoffMs=3000, multiplier=2)
 * - Poison pill handling: skip undecodable payloads after 5 retries
 * - Circuit breaker: threshold=6, resetTimeoutMs=6000
 * - ℹ️ Evolution: 100% optional fields suggests backward compatibility
 */
export class UserEventsDeletedConsumer {
  kafka;
  consumer;
  monitoring;
  auth = {};
  ...
  constructor(config) {
    ...
    this.resilienceConfig = {
      ...
      retry: {
        ...
        backoffMs: this.toNumber(config.retry?.backoffMs, process.env.KAFKA_RETRY_BACKOFF_MS, 3000),
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'user.events.deleted';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'user.events.deleted-consumer',
      brokers
    };
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'kafka',
      consumer: 'UserEventsDeletedConsumer',
      ...data
    };
  ...
  async handleMessage(payload) {
    ...
    const safeEvent = maskPII(event, ['userId']);
