...
/**
 * MQTT Consumer for sensor/events/alert
 * Purpose: Event channel: sensor/events/alert
 *
 * Governance:
 * - Topic: unknown
 * - QoS: 1 (At least once)
 * - Retained publish flag (default): Yes
 * - Clean Session: Yes
 ...
 * - PII fields: None
 */
export class SensorEventsAlertConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'sensor/events/alert-consumer-' + Math.random().toString(16).slice(2, 10);
    ...
    this.consumerOptions = {
      ...
      retained: normalizedConfig.retained == null ? true : normalizedConfig.retained === true
    };
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'SensorEventsAlertConsumer',
      ...data
    };
