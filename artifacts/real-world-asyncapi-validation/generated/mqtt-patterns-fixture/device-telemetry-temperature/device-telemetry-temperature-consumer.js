...
/**
 * MQTT Consumer for device/telemetry/temperature
 * Purpose: Event channel: device/telemetry/temperature
 *
 * Governance:
 * - Topic: unknown
 * - QoS: 1 (At least once)
 ...
 * - PII fields: None
 */
export class DeviceTelemetryTemperatureConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'device/telemetry/temperature-consumer-' + Math.random().toString(16).slice(2, 10);
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'DeviceTelemetryTemperatureConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs) {
    ...
    metrics?.incrementMessagesProcessed?.({
      transport: 'mqtt',
      topic: 'unknown',
      event: 'device/telemetry/temperature'
    });
