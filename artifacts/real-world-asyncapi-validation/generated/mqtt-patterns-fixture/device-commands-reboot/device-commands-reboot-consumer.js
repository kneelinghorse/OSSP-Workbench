...
/**
 * MQTT Consumer for device/commands/reboot
 * Purpose: Event channel: device/commands/reboot
 *
 * Governance:
 * - Topic: unknown
 * - QoS: 2 (Exactly once)
 ...
 * - PII fields: None
 */
export class DeviceCommandsRebootConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'device/commands/reboot-consumer-' + Math.random().toString(16).slice(2, 10);
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'DeviceCommandsRebootConsumer',
      ...data
    };
