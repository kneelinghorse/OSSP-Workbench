...
/**
 * Consumer for smartylighting.streetlights.1.0.action.{streetlightId}.turn.off
...
export class SmartylightingStreetlights10ActionStreetlightIdTurnOffConsumer {
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'smartylighting.streetlights.1.0.action.{streetlightId}.turn.off';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'smartylighting.streetlights.1.0.action.{streetlightId}.turn.off-consumer',
      brokers
    };
