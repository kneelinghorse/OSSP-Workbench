...
/**
 * Consumer for smartylighting.streetlights.1.0.action.{streetlightId}.dim
...
export class SmartylightingStreetlights10ActionStreetlightIdDimConsumer {
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'smartylighting.streetlights.1.0.action.{streetlightId}.dim';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'smartylighting.streetlights.1.0.action.{streetlightId}.dim-consumer',
      brokers
    };
