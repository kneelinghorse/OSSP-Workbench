  ...
  async start() {
    return new Promise((resolve, reject) => {
      this.client = mqtt.connect(this.brokerUrl, {
        clientId: 'smartylighting/streetlights/1/0/action/{streetlightId}/dim-consumer-' + Math.random().toString(16).substr(2, 8),
        clean: true,
        qos: 0
      });
      ...
      this.client.on('connect', () => {
        ...
        this.client.subscribe('smartylighting/streetlights/1/0/action/{streetlightId}/dim', { qos: 0 }, (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
