...
/**
 * MQTT Consumer for smartylighting/streetlights/1/0/action/{streetlightId}/dim
 * Purpose: Event channel: smartylighting/streetlights/1/0/action/{streetlightId}/dim
 *
 * Governance:
 ...
 * - Error topic: smartylighting/streetlights/1/0/action/{streetlightId}/dim/errors
 * - Last Will enabled: No
 * - PII fields: None
 */
export class SmartylightingStreetlights10ActionStreetlightIdDimConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'smartylighting/streetlights/1/0/action/{streetlightId}/dim-consumer-' + Math.random().toString(16).slice(2, 10);
    ...
    this.errorHandling = {
      ...
      errorTopic: normalizedConfig.errorTopic || process.env.MQTT_ERROR_TOPIC || 'smartylighting/streetlights/1/0/action/{streetlightId}/dim/errors',
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'SmartylightingStreetlights10ActionStreetlightIdDimConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs) {
    ...
    metrics?.incrementMessagesProcessed?.({
      transport: 'mqtt',
      topic: 'smartylighting/streetlights/1/0/action/{streetlightId}/dim',
      event: 'smartylighting/streetlights/1/0/action/{streetlightId}/dim'
    });
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.client.on('connect', () => {
        ...
        this.client?.subscribe('smartylighting/streetlights/1/0/action/{streetlightId}/dim', { qos: this.consumerOptions.qos }, (err) => {
          if (err) {
            if (!settled) {
              settled = true;
              reject(err);
