...
/**
 * MQTT Consumer for smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured
 * Purpose: The topic on which measured values may be produced and consumed.
 *
 * Governance:
 ...
 * - Error topic: smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured/errors
 * - Last Will enabled: No
 * - PII fields: None
 */
export class SmartylightingStreetlights10EventStreetlightIdLightingMeasuredConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured-consumer-' + Math.random().toString(16).slice(2, 10);
    ...
    this.errorHandling = {
      ...
      errorTopic: normalizedConfig.errorTopic || process.env.MQTT_ERROR_TOPIC || 'smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured/errors',
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'SmartylightingStreetlights10EventStreetlightIdLightingMeasuredConsumer',
      ...data
    };
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.client.on('connect', () => {
        ...
        this.client?.subscribe('smartylighting/streetlights/1/0/event/{streetlightId}/lighting/measured', { qos: this.consumerOptions.qos }, (err) => {
          if (err) {
            if (!settled) {
              settled = true;
              reject(err);
