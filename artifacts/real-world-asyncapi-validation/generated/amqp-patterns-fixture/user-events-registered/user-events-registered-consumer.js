...
/**
 * AMQP Consumer for user.events.registered
 * Purpose: Event channel: user.events.registered
 *
 * Governance:
 ...
 * - Connection retry: fixed (maxRetries=5, backoffMs=1000, multiplier=1)
 * - Auth support: Username/Password, mTLS (Client Certificates)
 * - PII fields: [userId, email]
 * - DLQ configured: ✅ Yes
 */
export class UserEventsRegisteredConsumer {
  connection = null;
  channel = null;
  channelPool = [];
  connectionUrl;
  ...
  buildConnectOptions() {
    return {
      ...
      clientProperties: {
        connection_name: 'UserEventsRegisteredConsumer'
      },
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'amqp',
      consumer: 'UserEventsRegisteredConsumer',
      ...data
    };
  ...
  async start() {
    ...
    const queue = 'user.registered.queue';
  ...
  async handleMessage(msg) {
    ...
    const safeEvent = maskPII(event, ['userId', 'email']);
  ...
  async sendToDLQ(channel, msg, error) {
    ...
    const dlqQueue = 'user.events.dlq';
    ...
    await activeChannel.sendToQueue(dlqQueue, msg.content, {
      headers: {
        ...
        'x-original-queue': 'user.registered.queue',
