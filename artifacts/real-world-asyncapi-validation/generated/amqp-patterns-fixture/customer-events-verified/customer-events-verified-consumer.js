...
/**
 * AMQP Consumer for customer.events.verified
 * Purpose: Event channel: customer.events.verified
 *
 * Governance:
 * - Exchange: customer.events
 * - Queue: customer.verified.queue
 * - Routing Key: customer.verified
 * - Durable: Yes
 * - Prefetch (default): 1
 ...
 * - PII fields: [customerId]
 * - DLQ configured: ⚠️ No
 */
export class CustomerEventsVerifiedConsumer {
  connection = null;
  channel = null;
  channelPool = [];
  connectionUrl;
  ...
  buildConnectOptions() {
    return {
      ...
      clientProperties: {
        connection_name: 'CustomerEventsVerifiedConsumer'
      },
  ...
  async prepareChannel(channel, queue) {
    ...
    await channel.bindQueue(queue, exchange, 'customer.verified');
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'amqp',
      consumer: 'CustomerEventsVerifiedConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs) {
    ...
    metrics?.incrementMessagesProcessed?.({
      transport: 'amqp',
      queue: 'customer.verified.queue',
      event: 'customer.events.verified'
    });
  ...
  async start() {
    ...
    const queue = 'customer.verified.queue';
  ...
  async handleMessage(msg) {
    ...
    const safeEvent = maskPII(event, ['customerId']);
