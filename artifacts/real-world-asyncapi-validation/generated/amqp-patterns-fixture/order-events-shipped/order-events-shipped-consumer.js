...
/**
 * AMQP Consumer for order.events.shipped
 * Purpose: Event channel: order.events.shipped
 *
 * Governance:
 ...
 * - Connection retry: none (maxRetries=3, backoffMs=60000, multiplier=1)
 * - Auth support: Username/Password, mTLS (Client Certificates)
 * - PII fields: [shippedAt]
 * - DLQ configured: ⚠️ No
 */
export class OrderEventsShippedConsumer {
  connection = null;
  channel = null;
  channelPool = [];
  connectionUrl;
  ...
  buildConnectOptions() {
    return {
      ...
      clientProperties: {
        connection_name: 'OrderEventsShippedConsumer'
      },
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'amqp',
      consumer: 'OrderEventsShippedConsumer',
      ...data
    };
  ...
  async start() {
    ...
    const queue = 'order.shipped.queue';
  ...
  async handleMessage(msg) {
    ...
    const safeEvent = maskPII(event, ['shippedAt']);
