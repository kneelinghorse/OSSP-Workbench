...
/**
 * MQTT Consumer for devices/{deviceId}/telemetry
 * Purpose: Device telemetry data stream
 *
 * Governance:
 * - Topic: unknown
 * - QoS: 1 (At least once)
 ...
 * - Error topic: unknown/errors
 * - Last Will enabled: No
 * - PII fields: [location.latitude, location.longitude]
 */
export class DevicesDeviceIdTelemetryConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'devices/{deviceId}/telemetry-consumer-' + Math.random().toString(16).slice(2, 10);
    this.monitoring = normalizedConfig.monitoring || { enabled: false };
    this.auth = normalizedConfig.auth || {};

    this.consumerOptions = {
      qos: this.normalizeQos(normalizedConfig.qos ?? process.env.MQTT_QOS ?? 1),
      cleanSession: (
    ...
    this.errorHandling = {
      ...
      errorTopic: normalizedConfig.errorTopic || process.env.MQTT_ERROR_TOPIC || 'unknown/errors',
  ...
  buildWillOptions(overrideWill = null) {
    ...
    return {
      ...
      qos: this.normalizeQos(configuredWill.qos ?? 1),
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'DevicesDeviceIdTelemetryConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs) {
    ...
    metrics?.incrementMessagesProcessed?.({
      transport: 'mqtt',
      topic: 'unknown',
      event: 'devices/{deviceId}/telemetry'
    });
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.client.on('connect', () => {
        ...
        this.client?.subscribe('unknown', { qos: this.consumerOptions.qos }, (err) => {
          if (err) {
            if (!settled) {
              settled = true;
              reject(err);
  ...
  async handleMessage(topic, message, context = {}) {
    ...
    const safeEvent = maskPII(event, ['location.latitude', 'location.longitude']);
