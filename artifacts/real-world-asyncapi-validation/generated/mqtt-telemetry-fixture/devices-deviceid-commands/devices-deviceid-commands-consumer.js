...
/**
 * MQTT Consumer for devices/{deviceId}/commands
 * Purpose: Device command channel
 *
 * Governance:
 * - Topic: unknown
 * - QoS: 2 (Exactly once)
 ...
 * - PII fields: None
 */
export class DevicesDeviceIdCommandsConsumer {
  client = null;
  brokerUrl;
  clientId;
  monitoring;
  ...
  constructor(config) {
    ...
    this.clientId = normalizedConfig.clientId || process.env.MQTT_CLIENT_ID || 'devices/{deviceId}/commands-consumer-' + Math.random().toString(16).slice(2, 10);
    this.monitoring = normalizedConfig.monitoring || { enabled: false };
    this.auth = normalizedConfig.auth || {};

    this.consumerOptions = {
      qos: this.normalizeQos(normalizedConfig.qos ?? process.env.MQTT_QOS ?? 2),
      cleanSession: (
  ...
  buildWillOptions(overrideWill = null) {
    ...
    return {
      ...
      qos: this.normalizeQos(configuredWill.qos ?? 2),
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'mqtt',
      consumer: 'DevicesDeviceIdCommandsConsumer',
      ...data
    };
  ...
  recordSuccessMetrics(durationMs) {
    ...
    metrics?.incrementMessagesProcessed?.({
      transport: 'mqtt',
      topic: 'unknown',
      event: 'devices/{deviceId}/commands'
    });
