...
/**
 * AMQP Consumer for notifications/email
 ...
 * - Connection retry: exponential (maxRetries=3, backoffMs=3600000, multiplier=2)
 * - Auth support: Username/Password, mTLS (Client Certificates)
 * - PII fields: [recipientEmail, recipientName]
 * - DLQ configured: ✅ Yes
 * - ℹ️ Retry: Retry policy without max attempts can cause infinite loops
 */
export class NotificationsEmailConsumer {
  connection = null;
  channel = null;
  channelPool = [];
  connectionUrl;
  ...
  constructor(config) {
    ...
    this.resilienceConfig = {
      retryPolicy: this.normalizeRetryPolicy(retryConfig.policy || process.env.AMQP_RETRY_POLICY || 'exponential'),
  ...
  buildConnectOptions() {
    return {
      ...
      clientProperties: {
        connection_name: 'NotificationsEmailConsumer'
      },
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'amqp',
      consumer: 'NotificationsEmailConsumer',
      ...data
    };
  ...
  async handleMessage(msg) {
    ...
    const safeEvent = maskPII(event, ['recipientEmail', 'recipientName']);
  ...
  async sendToDLQ(channel, msg, error) {
    ...
    await activeChannel.sendToQueue(dlqQueue, msg.content, {
      headers: {
        ...
        'x-original-queue': 'email.notifications',
