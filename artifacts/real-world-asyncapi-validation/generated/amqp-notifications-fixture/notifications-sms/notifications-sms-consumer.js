...
/**
 * AMQP Consumer for notifications/sms
 * Purpose: SMS notification queue
 *
 * Governance:
 ...
 * - Connection retry: fixed (maxRetries=3, backoffMs=1000, multiplier=1)
 * - Auth support: Username/Password, mTLS (Client Certificates)
 * - PII fields: [recipientPhone]
 * - DLQ configured: ✅ Yes
 * - ℹ️ Retry: Retry policy without max attempts can cause infinite loops
 */
export class NotificationsSmsConsumer {
  connection = null;
  channel = null;
  channelPool = [];
  connectionUrl;
  ...
  constructor(config) {
    ...
    this.resilienceConfig = {
      retryPolicy: this.normalizeRetryPolicy(retryConfig.policy || process.env.AMQP_RETRY_POLICY || 'fixed'),
  ...
  buildConnectOptions() {
    return {
      ...
      clientProperties: {
        connection_name: 'NotificationsSmsConsumer'
      },
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'amqp',
      consumer: 'NotificationsSmsConsumer',
      ...data
    };
  ...
  async handleMessage(msg) {
    ...
    const safeEvent = maskPII(event, ['recipientPhone']);
  ...
  async handleError(error, msg, channel) {
    ...
    // Route to DLQ: notifications.dlx
  ...
  async sendToDLQ(channel, msg, error) {
    ...
    const dlqQueue = 'notifications.dlx';
    await activeChannel.assertQueue(dlqQueue, { durable: true });

    await activeChannel.sendToQueue(dlqQueue, msg.content, {
      headers: {
        ...msg.properties.headers,
        'x-error': error.message,
        'x-original-queue': 'sms.notifications',
