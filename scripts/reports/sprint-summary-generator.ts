...
async function detectWorkspace(startDir: string): Promise<string> {
  ...
  for (const candidate of candidates) {
    const searchRoots = [candidate, path.join(candidate, 'OSSP-Workbench')];

    for (const root of searchRoots) {
      const directBacklogPath = path.join(root, 'missions', 'backlog.yaml');
      if (await fileExists(directBacklogPath)) {
        return root;
      }
      ...
      for (const archiveName of archiveEntries) {
        const archiveWorkspace = path.join(archivesDir, archiveName);
        const archiveBacklogPath = path.join(archiveWorkspace, 'missions', 'backlog.yaml');
        if (await fileExists(archiveBacklogPath)) {
