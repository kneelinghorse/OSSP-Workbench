...
missions/
└── sprint-01/
    ├── build_sprint.01.yaml
    └── research_sprint.01.yaml  (optional)
```

Notes:
- These sprint files are the source of truth for Sprint 01.
- The sprint backlog derives from `build_sprint.01.yaml`. Avoid duplicating planning logic elsewhere.
...
Location: `/missions/templates/RESEARCH_TEMPLATE.md` (optional template)

**For each research mission:**

1. **Copy the template** to `/missions/research/R1.#_[topic].md`
...
- Create `/missions/research/SYNTHESIS.md`
...
- [ ] Identify 3-5 research topics for Sprint 1
- [ ] Create R1.1, R1.2, R1.3... mission files using template
- [ ] Fill out all sections of each research mission
- [ ] Determine if synthesis document would be valuable
- [ ] If yes, create SYNTHESIS.md placeholder
...
1. Optionally create per-mission files (e.g., `/missions/sprint-01/build/B1.#_[component].yaml`). The canonical list of Sprint 01 build missions lives in `/missions/sprint-01/build_sprint.01.yaml`. The backlog should reflect this file rather than schemas or ad‑hoc lists.
...
4. **Reference Research Foundation** (if applicable)
   ...
   **Example with file path reference:**
   ...
   - **R1.1**: Please review algorithm research here: 
     `/missions/research/R1.1_algorithm_selection.md`
   - **R1.2**: Performance benchmarks documented here:
     `/missions/research/R1.2_performance_benchmarks.md`
   ```
...
└── sprint-01/
    ...
    └── build/
        ...
        ├── B1.4_[component].yaml
        └── B1.5_[component].yaml
```

**Final Checklist:**
- [ ] Sprint 1 folder structure is complete
- [ ] All research missions are detailed and ready to execute
