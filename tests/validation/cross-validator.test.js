...
describe('CrossValidator', () => {
  ...
  describe('URN Reference Validation', () => {
    test('should detect invalid URN format', () => {
      // The urn_references rule skips the manifest's own URN (line 201),
      // so we need an invalid URN in a *reference* field to trigger format validation.
      const manifest = {
        metadata: {
          urn: 'urn:proto:api:test.com/service@1.0.0'
        },
        dependencies: {
          depends_on: ['invalid-urn-format']
        }
      };
