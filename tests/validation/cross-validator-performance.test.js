...
describe('CrossValidator Performance Tests', () => {
  ...
  function generateTestManifests(count) {
    const manifests = [];
    // Only use kinds accepted by VALID_KINDS and NodeKind in the source:
    // api, api.endpoint, data, event, workflow, agent, integration, iam, semantic
    const protocolTypes = [
      'api', 'data', 'event', 'workflow', 'agent',
      'integration', 'iam', 'semantic'
    ];

    for (let i = 0; i < count; i++) {
      const protocolType = protocolTypes[i % protocolTypes.length];
      // URN format: urn:proto:<kind>:<authority>/<id>[@<version>]
      const urn = `urn:proto:${protocolType}:test.com/test-${i}@1.0.0`;
      ...
      const manifest = {
        ...
        dependencies: {
          depends_on: i > 0 ? [`urn:proto:${protocolTypes[(i-1) % protocolTypes.length]}:test.com/test-${i-1}@1.0.0`] : []
        }
      };
  ...
  describe('Performance Benchmarks', () => {
    ...
    test('should handle complex dependency chains efficiently', async () => {
      ...
      const circularManifest = {
        metadata: { urn: 'urn:proto:api:test.com/circular@1.0.0' },
        catalog: { endpoints: [{ path: '/circular', method: 'GET' }] },
        dependencies: {
          depends_on: ['urn:proto:data:test.com/test-49@1.0.0']
        }
      };

      // Add circular dependency
      const lastManifest = manifests[manifests.length - 1];
      lastManifest.dependencies.depends_on.push('urn:proto:api:test.com/circular@1.0.0');

      graph.addNode('urn:proto:api:test.com/circular@1.0.0', 'api', circularManifest);
      ...
      // Short cycles (length <= 3) are classified as errors, not warnings in the source
