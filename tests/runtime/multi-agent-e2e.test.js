...
describe('Multi-Agent E2E Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const registryConfig = {
      ...
      dataDir: join(__dirname, `../../data/test-registry-${testRunId}`)
    };

    // Initialize components
    registry = createURNRegistry(registryConfig);
    await registry.initialize();

    discovery = createAgentDiscoveryService({
      ...TEST_CONFIG.discovery,
  ...
  describe('MCP Tool Execution Integration', () => {
    ...
    it('should handle MCP tool execution timeouts', async () => {
      ...
      expect(endTime - startTime).toBeLessThan(2500); // Should timeout quickly
    });
  });
  ...
  describe('Complete Demo Integration', () => {
    ...
    it('should handle demo failures gracefully', async () => {
      ...
      const mockRegistry = {
        ...
        registerAgent: jest.fn().mockRejectedValue(new Error('Simulated registry failure')),
        cleanup: jest.fn()
      };
