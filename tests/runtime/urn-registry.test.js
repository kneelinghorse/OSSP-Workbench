...
describe('URN Registry', () => {
  ...
  describe('Agent Registration', () => {
    ...
    test('should store agent data to file', async () => {
      ...
      // Source replaces ALL non-alphanumeric chars with '_', so '-' becomes '_'
      const expectedFilename = 'urn_agent_ai_ml_agent_1_0_0.json';
      const expectedPath = join(testDataDir, 'agents', expectedFilename);

      // Source uses JSON.stringify with 2-space indent, so check for formatted JSON
  ...
  describe('Convenience Functions', () => {
    ...
    test('should get agent using convenience function', async () => {
      // The getAgent convenience function creates a new registry instance each time,
      // so it won't find agents registered by a different instance (in-memory only).
      // Verify it initializes and returns null for a non-existent agent.
  ...
  describe('Error Handling', () => {
    test('should handle file system errors during registration', async () => {
      ...
      // Mock writeFile to reject after initialization
      fs.writeFile.mockRejectedValue(new Error('Disk full'));

      await expect(registry.registerAgent(mockAgentData))
        .rejects.toThrow(URNError);

      // Restore writeFile mock and prevent afterEach shutdown from failing
      fs.writeFile.mockResolvedValue();
    });
    ...
    test('should handle index save errors', async () => {
      ...
      // After shutdown fails, isInitialized is still true due to error before line 503.
      // Restore writeFile and manually clean up to prevent afterEach double-shutdown failure.
      fs.writeFile.mockResolvedValue();
      registry = null;
    });
  });
