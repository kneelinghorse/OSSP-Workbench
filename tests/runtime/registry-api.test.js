...
describe('Registry API Server', () => {
  ...
  describe('Domain Endpoint', () => {
    ...
    test('should route domain URL to get-agent handler due to routing order', async () => {
      // The route /api/v1/agents/(.+) matches before /api/v1/agents/domain/(.+)
      // so domain requests are treated as agent lookups with urn="domain/ai"
      mockDiscoveryService.getAgent.mockResolvedValue(null);
      ...
      // Returns 404 because "domain/ai" is not a valid agent URN
      expect(response.statusCode).toBe(404);
      expect(response.body.error).toBe('Agent not found');
    });

    test('should handle domain URL errors as get-agent errors', async () => {
      mockDiscoveryService.getAgent.mockRejectedValue(
        new URNFormatError('Invalid URN format')
      );
  ...
  describe('Capability Endpoint', () => {
    ...
    test('should route capability URL to get-agent handler due to routing order', async () => {
      // The route /api/v1/agents/(.+) matches before /api/v1/agents/capability/(.+)
      // so capability requests are treated as agent lookups with urn="capability/ml-inference"
      mockDiscoveryService.getAgent.mockResolvedValue(null);
      ...
      // Returns 404 because "capability/ml-inference" is not a valid agent URN
      expect(response.statusCode).toBe(404);
      expect(response.body.error).toBe('Agent not found');
    });
  });
