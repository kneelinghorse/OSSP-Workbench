...
describe('Trace Entry', () => {
  ...
  test('should convert to JSON', () => {
    ...
    expect(json.duration).toBeGreaterThanOrEqual(1);
...
describe('Logger Manager', () => {
  ...
  describe('Manager Events', () => {
    test('should emit log events from managed loggers', () => {
      ...
      // Ensure manager logging is enabled
      manager.enableLogging = true;

      // Diagnostic: see if logger itself emits
      let loggerEmitted = false;
      logger.on('log', () => { loggerEmitted = true; });

      logger.info('Test message');

      expect(loggerEmitted).toBe(true);
