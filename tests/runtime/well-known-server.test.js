...
describe('Well-Known Server', () => {
  ...
  describe('Request Handling', () => {
    ...
    test('should handle URN resolution error', async () => {
      // Mock URN resolver to throw resolution error with correct error name
      const resolutionError = new Error('Agent not found');
      resolutionError.name = 'URNResolutionError';
      server.urnResolver.resolveAgentUrn = jest.fn().mockRejectedValue(resolutionError);
    ...
    test('should handle server errors', async () => {
      // Mock URN resolver to throw error (which is awaited inside _handleCapabilitiesList)
      // The error gets wrapped by _handleCapabilitiesList into WellKnownValidationError
      // which propagates out of handleRequest (return without await)
      server.urnResolver.discoverCapabilities = jest.fn().mockRejectedValue(
      ...
      await expect(server.handleRequest(request)).rejects.toThrow(WellKnownValidationError);
      await expect(server.handleRequest(request)).rejects.toThrow('Failed to discover capabilities: Internal server error');
    });
  });
  ...
  describe('Integration with ACM Generator and URN Resolver', () => {
    ...
    test('should use URN resolver for URN resolution', async () => {
      ...
      expect(resolveSpy).toHaveBeenCalledWith(urn);
    });
  });
  ...
  describe('Error Handling', () => {
    test('should wrap non-well-known errors', async () => {
      ...
      // Mock URN resolver to throw a non-well-known error (awaited inside _handleCapabilitiesList)
      // The error gets wrapped by _handleCapabilitiesList into WellKnownValidationError
      // which propagates out of handleRequest (return without await)
      server.urnResolver.discoverCapabilities = jest.fn().mockRejectedValue(
      ...
      await expect(server.handleRequest(request)).rejects.toThrow(WellKnownValidationError);
      await expect(server.handleRequest(request)).rejects.toThrow('Failed to discover capabilities: Unexpected error');
    });
    ...
    test('should preserve error context in logs', async () => {
      ...
      // Mock URN resolver to throw error (awaited inside _handleCapabilitiesList)
      // The error gets wrapped into WellKnownValidationError and propagates
      // out of handleRequest (return without await)
      serverWithLogging.urnResolver.discoverCapabilities = jest.fn().mockRejectedValue(
      ...
      // The error propagates out of handleRequest since _handleCapabilitiesList
      // is returned without await
      await expect(serverWithLogging.handleRequest(request)).rejects.toThrow(WellKnownValidationError);
