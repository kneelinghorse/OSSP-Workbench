  ...
  try {
    ...
    if (manifest.service) {
      if (!manifest.service.name) {
        blockingErrors.push('service.name');
      }
      if (!manifest.service.urn) {
        blockingErrors.push('service.urn');
      }
    }

    if (blockingErrors.length > 0 && !options.force) {
      console.error(`Cannot approve manifest with validation errors: ${blockingErrors.join(', ')}`);
      process.exit(1);
      return null;
    }
    ...
    const updated = {
      ...
      metadata: {
        ...
        state_history: [
          ...
          { from: status, to: 'approved', at: approvedAt, by: approvedBy, forced: options.force === true }
        ]
      }
    };
