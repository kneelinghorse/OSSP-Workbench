...
/**
 * Consumer for payments.protobuf
 * Purpose: Event channel: payments.protobuf
 *
 * Governance:
 * - PII fields: None
 * - DLQ configured: ⚠️ No
 ...
 * - Serialization: PROTOBUF via Confluent Schema Registry (application/protobuf)
 * - Schema evolution: BACKWARD compatibility with optional AVRO reader schema override
 * - ℹ️ Evolution: 50% optional fields provides moderate flexibility
 */
export class PaymentsProtobufConsumer {
  kafka;
  consumer;
  schemaRegistry = null;
  ...
  constructor(config) {
    ...
    this.schemaRegistryConfig = {
      ...
      compatibility: this.normalizeSchemaCompatibility(
        config.schemaRegistry?.compatibility || process.env.SCHEMA_REGISTRY_COMPATIBILITY || 'BACKWARD'
      ),
  ...
  normalizeSchemaCompatibility(value) {
    ...
    return supported.has(normalized) ? normalized : 'BACKWARD';
  }
  ...
  async deserializeMessageValue(value) {
    ...
    try {
      ...
      throw new Error(`Failed to deserialize Kafka payload as PROTOBUF/JSON: ${error?.message || String(error)}`);
    }
  }

  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'payments.protobuf-consumer',
      brokers
    };
  ...
  async log(level, event, data = {}) {
    const payload = {
      ...
      event,
      transport: 'kafka',
      consumer: 'PaymentsProtobufConsumer',
      ...data
    };
