...
/**
 * Consumer for smartylighting.streetlights.1.0.event.{streetlightId}.lighting.measured
...
export class SmartylightingStreetlights10EventStreetlightIdLightingMeasuredConsumer {
  ...
  resolveMessageIdentity(payload) {
    const topic = payload?.topic || 'smartylighting.streetlights.1.0.event.{streetlightId}.lighting.measured';
  ...
  buildKafkaConfig(brokers) {
    const kafkaConfig = {
      clientId: 'smartylighting.streetlights.1.0.event.{streetlightId}.lighting.measured-consumer',
      brokers
    };
