...
describe('RegistrationPipeline', () => {
  ...
  describe('loadState', () => {
    ...
    test('should return default DRAFT state for non-existent manifest', async () => {
  ...
  describe('getCurrentState', () => {
    ...
    test('should return DRAFT for non-existent manifest', async () => {
      const currentState = await pipeline.getCurrentState('non-existent');
      expect(currentState).toBe(STATES.DRAFT);
    });
  });
  ...
  describe('invalid transitions', () => {
    ...
    test('should reject transition from REGISTERED (terminal state)', async () => {
      ...
      await expect(pipeline.submitForReview(manifestId))
        .rejects.toThrow("Event 'submit_for_review' not allowed in state 'REGISTERED'");
    });
  });
