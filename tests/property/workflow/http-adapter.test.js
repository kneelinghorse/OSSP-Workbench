  ...
  describe('Input Validation Properties', () => {
    ...
    it('should always reject invalid timeout values', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.timeout && ...)` so falsy values (0, null, undefined, '')
        // are treated as "not set" and skip validation entirely (valid).
        // Only truthy non-number values or truthy negative numbers trigger errors.
  ...
  describe('Error Handling Properties', () => {
    ...
    it('should always retry on server errors but not client errors', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        // For 5xx errors, retries happen (server error path stores lastError and loops)
        // For 4xx errors, the source throws immediately inside the try block,
        // but the catch doesn't check for status-based errors, so it still retries.
        // Both paths result in 3 fetch calls (1 initial + 2 retries).
  ...
  describe('Timeout Properties', () => {
    it('should always timeout after specified duration', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // The mock must respect the AbortSignal to properly simulate timeout.
        // It returns a never-resolving promise that rejects when signal is aborted.
        global.fetch = jest.fn().mockImplementation((url, options) => {
          return new Promise((resolve, reject) => {
            const timer = setTimeout(resolve, timeout + 5000); // Very long delay
            if (options?.signal) {
