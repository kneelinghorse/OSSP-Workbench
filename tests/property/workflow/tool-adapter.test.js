...
/**
 * Helper to create a context compatible with adapter.execute().
 * processResult() calls context.getElapsedTime() which requires a WorkflowContext.
 */
function createContext(overrides = {}) {
  return new WorkflowContext(overrides);
}
...
describe('Tool Adapter Property Tests', () => {
  ...
  describe('Input Validation Properties', () => {
    ...
    it('should always reject invalid timeout values', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.timeout !== undefined)` guard, so undefined
        // skips validation entirely (valid). All other invalid values are caught.
    ...
    it('should always reject invalid max retries values', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.maxRetries !== undefined)` guard, so undefined
        // skips validation entirely (valid). All other invalid values are caught.
  ...
  describe('Tool Registry Properties', () => {
    ...
    it('should always reject invalid tool registrations', async () => {
      for (let i = 0; i < 20; i++) {
        ...
        // Source only checks `typeof tool.execute !== 'function'`:
        // - Objects without execute: throws "Tool must have execute function"
        // - Objects with non-function execute: throws "Tool must have execute function"
        // - null/undefined/primitives: throws TypeError (cannot read property of null/undefined)
        // - Objects WITH valid execute: succeeds (even if missing name)
        const invalidTools = [
          { name: 'invalid1' },                          // Missing execute -> custom error
          { name: 'invalid3', execute: 'not_a_function' }, // Non-function execute -> custom error
          null,                                            // TypeError
          undefined,                                       // TypeError
          'string',                                        // TypeError (cannot read .execute of string)
          123                                              // TypeError
        ];
        ...
        invalidTools.forEach((invalidTool, j) => {
          try {
            ...
            // All should throw - either "Tool must have execute function"
            // or a TypeError for null/undefined/primitive values
    ...
    it('should always handle tool execution errors', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        const input = {
          tool: 'failing_tool',
          args: { test: `value_${i}` },
          maxRetries: 0  // Disable retries; this test focuses on error handling, not retry logic
        };
  ...
  describe('Retry Logic Properties', () => {
    it('should always retry on transient failures', async () => {
      ...
      // Source uses exponential backoff: ~1s + ~2s per iteration (~3s each).
      // Reduce iterations to stay within jest timeout.
      for (let i = 0; i < 3; i++) {
        attemptCount = 0;
    ...
    it('should always timeout after specified duration', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const input = {
          ...
          timeout,
          maxRetries: 0  // Disable retries; this test focuses on timeout behavior
        };
  ...
  describe('Default Tools Properties', () => {
    ...
    it('should always execute delay tool correctly', async () => {
      for (let i = 0; i < 10; i++) {
        ...
        // Allow small negative tolerance (~2ms) since setTimeout doesn't guarantee
        // exact timing and performance.now() can measure slightly less than requested.
        expect(duration).toBeGreaterThanOrEqual(delay - 2);
