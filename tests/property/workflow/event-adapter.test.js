...
function createContext(overrides = {}) {
  const ctx = new WorkflowContext(overrides);
  return ctx;
}
...
describe('Event Adapter Property Tests', () => {
  ...
  describe('Input Validation Properties', () => {
    ...
    it('should always reject invalid priority values', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.priority !== undefined)` so undefined skips validation.
        // All other non-undefined invalid values should trigger an error.
    ...
    it('should always reject invalid TTL values', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.ttl !== undefined)` so undefined skips validation.
        // All other non-undefined invalid values should trigger an error.
    ...
    it('should always reject invalid routing keys', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // Source uses `if (input.routingKey && typeof input.routingKey !== 'string')`
        // so falsy values (false, 0, null, undefined, '') skip the check.
        // Only truthy non-string values trigger errors.
  ...
  describe('Event Emission Properties', () => {
    ...
    it('should always handle event bus errors gracefully', async () => {
      // Create adapter with an invalid event bus (must be truthy but non-functional
      // so it isn't replaced by the default bus via the || fallback)
      const invalidAdapter = new EventAdapter({
        eventBus: { broken: true }
      });
