...
describe('Catalog Index Adapter Property Tests', () => {
  ...
  describe('Manifest Validation Properties', () => {
    ...
    it('should always validate URN format in manifests', async () => {
      // Manifests with falsy URN values should fail with "Manifest must have a URN"
      const falsyUrnManifests = [
      ...
      for (let i = 0; i < falsyUrnManifests.length; i++) {
        const manifest = falsyUrnManifests[i];
        const result = adapter.validateManifest(manifest);
      ...
      // Manifests with truthy but malformed URN values still pass
      // validateManifest only checks for presence, not format
      const truthyInvalidUrnManifests = [
      ...
      for (let i = 0; i < truthyInvalidUrnManifests.length; i++) {
        const manifest = truthyInvalidUrnManifests[i];
        const result = adapter.validateManifest(manifest);

        // These have all required fields present, so validation passes
        expect(result.valid).toBe(true);
        expect(result.errors).toHaveLength(0);
      }
    });
  ...
  describe('Query Operations Properties', () => {
    it('should always find manifests by namespace', async () => {
      ...
      for (let i = 0; i < namespaces.length; i++) {
        ...
        expect(result.results.length).toBe(10); // 30 total / 3 namespaces = 10 each
        result.results.forEach(manifest => {
    ...
    it('should always find manifests by type', async () => {
      ...
      for (let i = 0; i < types.length; i++) {
        ...
        expect(result.results).toBeDefined();
        expect(Array.isArray(result.results)).toBe(true);
        expect(result.results.length).toBe(10); // 30 total / 3 types = 10 each
        result.results.forEach(manifest => {
