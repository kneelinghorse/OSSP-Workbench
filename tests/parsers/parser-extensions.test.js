...
describe('ParserExtensions', () => {
  ...
  describe('detectCircularRefs', () => {
    it('should detect circular references in fixture', async () => {
      ...
      // Default allowCircular is false, so detectCircularRefs throws on circular refs
      const permissiveExt = new ParserExtensions({
        basePath: fixturesDir,
        refTimeout: 5000,
        allowCircular: true
      });

      const result = permissiveExt.detectCircularRefs(json);
